...
def get_item_compare(value, dados_banco, column, get_one = True):
    resultados = []
    
    if value is not None:
        for dado in dados_banco:
            if compare_item_includes(value, dado):
                resultados.append(dado)
                if get_one:
                    break
...
def compare_item_includes(value_1, value_2, normatizar = False):
    if normatizar:
